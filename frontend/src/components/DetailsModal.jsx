...
const DetailsModal = ({ open, onClose, formData = {}, onSave }) => {
  ...
  // Run Book form state
  const [runBookErrorDescription, setRunBookErrorDescription] = useState(formData.runBookErrorDescription || "");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to add action: ${errorText || response.status}`);
      }
  ...
  return (
    ...
    >
      <DialogContent sx={{ p: 0, display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        {/* Header with Tabs */}
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", p: 2, borderBottom: 1, borderColor: "divider", position: "relative" }}>
          <Box sx={{ position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
            <Tabs value={activeTab} onChange={(e, newValue) => setActiveTab(newValue)}>
              <Tab label="Run Book" />
              <Tab label="Actions" icon={<AutoAwesomeIcon />} iconPosition="start" />
            </Tabs>
          </Box>
        ...
        <Box sx={{ display: "flex", flex: 1, minHeight: 0 }}>
          ...
          <Box sx={{ width: "50%", overflowY: "auto", p: 2, bgcolor: "background.paper" }}>
            {protocols.map((protocol) => (
              <Card key={protocol.id} sx={{ mb: 2, bgcolor: "grey.50", borderRadius: 2, border: 1, borderColor: "grey.50" }}>
                <CardContent>
          ...
          {activeTab === 0 && (
            <Box sx={{ width: "50%", display: "flex", flexDirection: "column", flex: 1, minHeight: 0, bgcolor: "background.paper" }}>
              ...
              <Box sx={{ flex: 1, overflowY: "auto", overflowX: "visible", p: 3, position: "relative" }}>
                <Box sx={{ mb: 3 }}>
                ...
                <Box sx={{ mb: 3 }}>
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1 }}>
                    ...
                    <ButtonGroup size="small">
                      <Button
                        variant={correctiveMeasureMode === "prompt" ? "contained" : "outlined"}
                        onClick={() => setCorrectiveMeasureMode("prompt")}
                      >
                        Prompt
                      </Button>
                      <Button
                        variant={correctiveMeasureMode === "code" ? "contained" : "outlined"}

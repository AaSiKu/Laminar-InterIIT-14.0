...
class JsonIndexingApp:
    def __init__(
        ...
        json_file_path: str = "mongo_data.json",  # Backward compatibility
        host: str = "0.0.0.0",
        port: int = 8000,
        ...
        if json_sources:
            ...
            self.json_sources = {"default": json_file_path}
...
# ===== ALTERNATIVE: SIMPLE INLINE APPROACH =====
def run_simple_json_indexer():
    """
    Simpler approach without classes - define everything inline
    """
    
    # Read JSON file
    json_source = pw.io.fs.read(
        path="mongo_data.json",
        format="binary",
        with_metadata=True,
    )
    ...
    @pw.udf
    def parse_json_to_entries(data: bytes) -> list[dict]:
        """Parse JSON and return list of entries"""
        try:
    ...
    @pw.udf
    def extract_first_column(entry: dict) -> str:
        """Extract first column value for semantic search"""
        if entry:
            first_key = list(entry.keys())[0]
            return str(entry.get(first_key, ""))
        return ""
    
    # Process documents
    documents = json_source.select(
        text=extract_first_column(parse_json_to_entries(pw.this.data)),
        metadata=pw.this.path
    )
    ...
    # Start server
    server = DocumentStoreServer("0.0.0.0", 8000, document_store)
    server.run(with_cache=True, terminate_on_error=False)
...
if __name__ == "__main__":
    ...
    # Option 2: Multiple sources (only use if you have these files)
    # app = JsonIndexingApp(
    #     json_sources={
    #         "errors": "mongo_data.json",
    #         "runbooks": "runbooks.json",  # Optional
    #         "incidents": "incidents.json",  # Optional
    #     },
    #     host="0.0.0.0",
    #     port=8000,
    #     embedding_model="models/text-embedding-004",
